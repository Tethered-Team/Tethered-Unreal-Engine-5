...
ACombatCharacter::ACombatCharacter()
{
	...
	// Character should face its movement direction (not controller yaw)
...
void ACombatCharacter::DoMove(float Right, float Forward)
{
	...
	// Combine input so diagonals don't move faster
	const FVector2D Input2D(Right, Forward);
	if (Input2D.IsNearlyZero())
	{
		return;
	}

	// Use camera yaw so movement is relative to the screen
	const FRotator CamRot = FollowCamera ? FollowCamera->GetComponentRotation() : GetController()->GetControlRotation();
	const FRotator YawOnly(0.f, CamRot.Yaw, 0.f);

	// Camera-relative axes on the horizontal plane
	...
	// Build move direction and magnitude
	const FVector MoveDir = (CamForward * Input2D.Y + CamRight * Input2D.X).GetSafeNormal();
	const float   MoveMag = FMath::Clamp(Input2D.Size(), 0.f, 1.f);

	// While attacking (combo) or charging, allow rotation toward input even if movement is constrained
	const bool bWantsForcedRotation = (bIsChargingAttack || bIsAttacking);
	if (bWantsForcedRotation && !MoveDir.IsNearlyZero())
	{
		// If we're basically stationary, turn manually using the same turn rate as movement
		const float SpeedSq2D = GetCharacterMovement()->Velocity.SizeSquared2D();
		if (SpeedSq2D < 1.f)
		{
			const float DeltaTime = GetWorld()->GetDeltaSeconds();
			const float TurnSpeedDegPerSec = GetCharacterMovement()->RotationRate.Yaw;
			const FRotator DesiredYaw(0.f, MoveDir.Rotation().Yaw, 0.f);
			const FRotator NewYaw = FMath::RInterpConstantTo(GetActorRotation(), DesiredYaw, DeltaTime, TurnSpeedDegPerSec);
			SetActorRotation(NewYaw);
		}
	}

	// Apply movement; when moving, bOrientRotationToMovement will handle facing
	AddMovementInput(MoveDir, MoveMag);
}
